...
(function () {
  ...
  toto.onRebuild(function () {
    $overlayScreen = $('<toto-screen>').addClass('overlay').appendTo($('.proto-root'));
    $overlayScreen.hide();
  });
  ...
  toto.onScreenTransition(function (currentScreen, newScreen, options, callback) {
    ...
    if (options.sharedElements) {
      ...
      options.sharedElements.forEach(function (se) {
        ...
        performSharedElementTransition_(currentScreen, newScreen, fromId, toId, first ? callback : null);
        first = !first;
      });
    } else {
      callback();
  ...
  function performSharedElementTransition_(fromScreen, toScreen, fromLayerName, toLayerName, callback) {
    ...
    if (!$fromEl.length || !$toEl.length) {
      console.warn('A shared element transition from layer ' + fromLayerName + (' to ' + toLayerName + ' was not performed because either the ') + 'source or target layer was missing.');
      callback();
      return;
    }
    ...
    var $transitionElStart = $fromEl.clone().appendTo($overlayScreen);
    var $transitionElEnd = $toEl.clone().appendTo($overlayScreen);

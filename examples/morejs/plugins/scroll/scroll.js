...
(function () {
  ...
  toto.onLoadLayer(function (layerData, $layer, screen) {
    ...
    if (layerData.annotations.slidePager) {
      ...
      $layer.hotspot(function () {
        return screen.$$('#' + targetLayerName).pagerSlidePager(slidePosition);
      });
    }
  });
  ...
  function makePager($pager) {
    ...
    $pager.data('pageWidth', pageWidth).width(pageWidth * $pages.length).addClass('pager').on('mousedown touchstart', function (ev) {
  ...
  $(window).on('mousemove touchmove', function (ev) {
    ...
    if (pagingEngaged) {
      ...
      if (left % activePagerWidth != 0) {
        var velocity = (x_(evLast) - x_(evRecent)) / (evLast.timeStamp - evRecent.timeStamp + 1) * 1000;
  ...
  $(window).on('mousemove', function (ev) {
    ...
    if (scrollingEngaged) {
      var velocity = (vf_(evLast) - vf_(evRecent)) / (evLast.timeStamp - evRecent.timeStamp + 1) * 1000;
  ...
  function animateScrollSettle($scroller, startVelocity, vertical) {
    ...
    _currentScrollSettleAnimation = function currentScrollSettleAnimation() {
      ...
      if (Math.abs(velocity) > 0 && _currentScrollSettleAnimation) {
        requestAnimationFrame(_currentScrollSettleAnimation);
      }
    };
